...
//cart functions ------------------------------------------------------------------------------------------------------------
  ...
  const display = () => {
    const categorycontent = JSON.parse(localStorage.getItem('categorypagecontent'));
  ...
  if (ismenushow || issearchscreenshow){
   ...
   const scrollfunctionleft = () => {
    ...
    const companycategorynamee = localStorage.getItem('companycategory')
...
return(
    ...
    <li key={index} onClick={()=> {selectedproduct(data); filterBestsellers(); handleRefresh();}}> 
...
{sizes.map((size,index) => (
  <option key={index} value={size}>{size}</option>
    ...
    <div className='profilecard'>
       ...
       <Link to={`/categorypage/Nike`} onClick={() => {filternikeproducts();setcompanynamenike();handleRefresh();}} className='searchlink'>
 <div className='nikecard'><img src={nikelogo} alt='nikelogo'></img><p>Nike</p></div>
 </Link>
 <Link to={`/categorypage/Puma`} onClick={() => {setcompanynamepuma();filterpumaproducts();handleRefresh();}} className='searchlink'>
 <div className='pumecard' ><img src={pumalogo} alt='pumalogo'></img><p>Puma</p></div>
 </Link>
 <Link to={`/categorypage/Vans`} onClick={() => {setcompanynamevans();filtervansproducts();handleRefresh();}} className='searchlink'>
 <div className='vanscard' ><img src={vanslogo} alt='vanslogo'></img><p>Vans</p></div>
 </Link>
 <Link to={`/categorypage/Adidas`} onClick={() => {setcompanynameadidas();filteradidasproducts();handleRefresh();}} className='searchlink'>
 <div className='adidascard' ><img src={adidaslogo} alt='adidaslogo'></img><p>Adidas</p></div>
 </Link>
 </div>
 </div>
<div className='categh1'>
<Link to="/">
<span><MdOutlineArrowBackIos /></span> </Link><h1 >{companycategorynamee}</h1></div>
<div className='categorypage'>
{categorycontent.map(product => (
 
   <div className='categorypageproductcard'>
    <div className='imagecontcont'>
    <Link to={`/product/${product.title}`}>
    <img src={product.img} onClick={()=> selectedproduct(product)}></img>
    </Link>
    </div>
    <div className='categorypagetitlepricecont'>
      <span>
    <h1>{product.title}</h1>   <p>{product.reviews}</p>
    </span>
    <p>Category : {product.category} ({product.color})</p>
    <h5 className='linethrough'>{product.prevPrice}</h5>
    <h5>${product.newPrice}</h5>
    </div>
    <div className='categorypagebuttons'>
    <select placeholder id='addtocartbtn' value='selectedsize' className='categorypageaddtocartbtn'onChange={(e)=> {getItems();addproducttoccart(product ,e.target.value)}} >
       <option value=""  selected>Add to Cart</option>
{sizes.map((size,index) => (
  <option key={index} value={size}>{size}</option>
))}
